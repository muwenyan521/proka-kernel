...
impl<'a> BMFParser<'a> {
    ...
    pub fn get_char_data(&self, unicode: u32) -> Option<&[u8]> {
        let mut slot = unicode % self.hash_slots;
        
        loop {
            let entry_start = (self.hash_start as usize) + (slot as usize * 6);
    ...
    pub fn get_grayscale_image(&self, unicode: u32) -> Option<Vec<Vec<u8>>> {
        ...
        let mut image = alloc::vec![alloc::vec![0u8; self.font_size as usize]; self.font_size as usize];
